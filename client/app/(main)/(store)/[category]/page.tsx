...
export default function CategoryPage() {
    ...
    const categoryHeroData: Record<string, CategoryHeroData> = {
        iphone: {
            title: "iPhone",
            description:
                "Khám phá dòng iPhone mới nhất với công nghệ tiên tiến và thiết kế đột phá",
            image:
                "https://images.unsplash.com/photo-1678685888221-cda773a3dcdb?w=1200&h=400&fit=crop",
            productsCount: 24,
        },
        macbook: {
            title: "MacBook",
            description:
                "Hiệu suất vượt trội với chip Apple Silicon, thiết kế mỏng nhẹ đẳng cấp",
            image:
        ...
        watch: {
            title: "Apple Watch",
            description:
                "Đồng hồ thông minh với tính năng sức khỏe và fitness toàn diện",
            image:
        ...
        airpods: {
            title: "AirPods",
            description:
                "Trải nghiệm âm thanh không dây tuyệt vời với chất lượng cao cấp",
            image:
    ...
    const currentBanners = categoryBanners[category] || [currentCategory.image];
    ...
    const filterBySeries = (products: Product[]) => {
        ...
        return products.filter((product) => {
            ...
            return product.name
                .toLowerCase()
                .includes(selectedSeries.toLowerCase().replace(" series", ""));
        });
    };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white dark:from-slate-950 dark:to-slate-900">
            ...
            <BannerCarousel
                ...
                autoplayDelay={5000}
                altPrefix={currentCategory.title}
            />
                ...
                {loading ? (
                    ...
                    >
                        {sortedProducts.map((product, index) => (
                            <Card
                                key={product.id}
                                className="group py-0 hover:shadow-2xl transition-all duration-300 overflow-hidden border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 animate-in fade-in slide-in-from-bottom-4"
                                style={{ animationDelay: `${index * 50}ms` }}
                            >
                                <CardHeader className="relative p-0 overflow-hidden">
                                    <Link href={`/products/${product.id}`}>
                                        <div className="relative bg-gradient-to-br from-slate-100 via-slate-50 to-slate-100 dark:from-slate-800 dark:via-slate-900 dark:to-slate-800 overflow-hidden">
                                            {product.image ? (
                                                <img
                                                    src={product.image}
                                                    alt={product.name}
